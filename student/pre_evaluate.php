...
session_start();
    ...
    if (isset($_POST["submit"])) {
        ...
        // SQL query to check if same evaluation records already submitted
        $sqlCheckExist = "SELECT * FROM eval_tbl WHERE faculty_id = '$faculty_id' AND course_id = '$course_id' AND student_id =" .  $_SESSION['student_id'] ;
        $resultCheckExist = $conn->query($sqlCheckExist);

        $numRows = $resultCheckExist->num_rows;

        if ($numRows >= 1) {
            // Records exist, do something
            $_SESSION['error'] = "You already submitted evaluation for this faculty and course!";
