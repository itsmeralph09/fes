<?php
	...
	if(isset($_GET['acad_id'])){
		...
		if ($row['results_release'] == "no") {
			$_SESSION['error'] = 'Results for this academic year is already hold!';
			mysqli_close($conn);
		...
		$sql = "UPDATE acad_yr_tbl
				SET results_release = 'no'
				WHERE acad_id = '$acad_id'";

		$result = mysqli_query($conn, $sql);

		if($result){
			$_SESSION['success'] = 'Academic year results hold successfully!';
			mysqli_close($conn);
			header('location: acad_yr.php');
			exit;
		} else{
			$_SESSION['error'] = 'Failed to hold results for this academic year!';
			mysqli_close($conn);
			header('location: acad_yr.php');
			exit;
		}
		...
		$_SESSION['error'] = 'Select academic year to hold results first!';
		mysqli_close($conn);
		header('location: acad_yr.php');
		exit;
	}
