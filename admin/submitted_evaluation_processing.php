...
while ($row = mysqli_fetch_assoc($query)) {
    ...
    $nestedData[] = $dateOnly;
    $nestedData[] = "<button class='btn btn-danger btn-sm' onclick='confirmDelete(\"" . $row['eval_id'] . "\", \"" . $row['student_name'] . "\", \"" . $row['course'] . "\", \"" . $row['class'] . "\")'><i class='fa fa-trash m-1'></i>Delete</button>";
