<?php
	...
	if(isset($_POST['edit'])){
		...
		if (empty($password)) {
			$sql = "UPDATE admin_tbl SET first_name = '$first_name', middle_name ='$middle_name', last_name = '$last_name',  ext_name = '$ext_name', email='$email' WHERE admin_id = '$admin_id'";
			$result = mysqli_query($conn, $sql);
			if($result){
						$_SESSION['success'] = 'Admin updated successfully, but password remain unchanged!';
					}	
		}else{
			if ($password==$confirm_password) {
				...
				$result = mysqli_query($conn, $sql);
					if($result || $result2){
						$_SESSION['success'] = 'Admin updated successfully, and password changed!';
					}
			}else{
